...
(function() {
    ...
    function setupRemind() {
        ...
        var content_overview = [
            ...
            '<p>&nbsp;&nbsp;&nbsp;&nbsp;<a id="reset_notice_status" href="javascript:;">重置状态</a>：此操作将已读公告标记为未读，下次请求后会重新展示公告。</p>',
            '</div>'
        ].join("");
        var content_remind = [
            '<div style="padding: 30px; line-height: 22px; font-weight: 300;">',
            '<form><a id="save_remind_email" class="submit-btn btn rbtn" href="javascript:;">保存</a><input style="display:inline-block;height:28px;color:#777;background:#fcfcfc;border:1px solid #CCC;border-radius:2px;box-shadow:inset 0 1px 2px rgba(0,0,0,.05);-webkit-transition:color .2s linear,border-color .3s linear;" id="set_remind_email" type="text" placeholder="邮箱" value="' + email + '"></form>',
            '<p><form><a id="save_remind_mobile" class="submit-btn btn rbtn" href="javascript:;">保存</a><input style="display:inline-block;height:28px;color:#777;background:#fcfcfc;border:1px solid #CCC;border-radius:2px;box-shadow:inset 0 1px 2px rgba(0,0,0,.05);-webkit-transition:color .2s linear,border-color .3s linear;" id="set_remind_mobile" type="text" placeholder="手机号" value="' + mobile + '"></form></p>',
            '<p>微信下载进度查询：</p>',
