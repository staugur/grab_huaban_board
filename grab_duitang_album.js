...
(function() {
     ...
     */
    function setupReceiveTo() {
        // 绑定全局方法
        //window.StorageMix = StorageMix;
        ...
        var content_overview = [
            ...
            '<p>提醒功能旨在提交远程下载后，查询下载进度并在下载完成发送邮箱、短信、微信等消息，以供用户下载。</p>',
            '<p>提醒邮箱：<scan id="overview_email">' + (es.get() || '未设置!</scan></p>'),
            '<p>提醒手机：<scan id="overview_mobile">' + (ms.get() || '未设置!</scan></p>'),
            '</div>'
        ].join("");
        var content_email = `
            <div style="padding: 30px; line-height: 22px; font-weight: 300;">
                <form>
                    <input class="ipt" id="set_remind_email" autocomplete="off" type="text" placeholder="邮箱地址">
                    <a id="save_remind_email" class="abtn abtn-w4" href="javascript:;"><u>保存</u></a>
                </form>
                <p><div id="save_remind_email_tip"></div></p>
            </div>
            <script>
                document.getElementById("save_remind_email").onclick = function() {
                    var value = document.getElementById("set_remind_email").value;
                    console.log(value);
                    var isEmail = /^[\\w.\\-]+@(?:[a-z0-9]+(?:-[a-z0-9]+)*\\.)+[a-z]{2,3}$/i;
                    if (!isEmail.test(value)) {
                        parent.layer.msg('请输入正确的邮箱地址');
                        return;
                    }
                    parent.storage.es.set(value);
                    parent.document.getElementById("save_remind_email_tip").html = '邮箱：' + value + '，设置成功！';
                    parent.document.getElementById("overview_email").html = value;
                }
            </script>
        `;
        var content_mobile = `
            <div style="padding: 30px; line-height: 22px; font-weight: 300;">
                <form>
                    <input class="ipt" id="set_remind_mobile" autocomplete="off" type="text" placeholder="手机号">
                    <a id="save_remind_mobile" class="abtn abtn-w4" href="javascript:;"><u>保存</u></a>
                </form>
                <p><div id="save_remind_mobile_tip"></div></p>
            </div>
            <script>
                document.getElementById("save_remind_mobile").onclick = function() {
                    var value = document.getElementById("set_remind_mobile").value;
                    console.log(value);
                    ...
                    parent.storage.ms.set(value);
                    parent.document.getElementById("save_remind_mobile_tip").html = "手机号：" + value + "，设置成功！";
                    parent.document.getElementById("overview_mobile").html = value;
                }
            </script>
        `;
        layer.tab({
    ...
    */
    ...
    if (album_id != undefined && /^[0-9]*$/.test(album_id)) {
        ...
        // 监听保存手机
        document.getElementById('save_remind_mobile').onclick = function() {
            var mobile = document.getElementById("set_remind_mobile").value
